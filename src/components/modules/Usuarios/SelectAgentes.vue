<template>
  <v-col v-if="cols || md" :cols="cols" :md="md" :offset-md="offsetMd">
    <v-autocomplete
      ...
      outlined
      label="Agente:"
      item-text="correoElectronico"
      :item-value="itemValue ? itemValue : 'id'"
      :rules="rules"
      :items="getUsuariosAgentes"
      :readonly="readonly"
      :loading="loading"
      :return-object="returnObject"
      :disabled="disabled || getUsuariosAgentes.length == 0"
    />
  </v-col>
  <v-autocomplete
    v-else
...
export default {
  ...
  computed: {
    ...
    ...mapGetters({
      getUsuariosAgentes: "usuariosController/getUsuariosAgentes",
    }),
